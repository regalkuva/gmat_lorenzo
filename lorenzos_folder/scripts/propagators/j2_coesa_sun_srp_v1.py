## ORBIT PROPAGATOR WITH J2 + ATMOSPHERIC DRAG + 3rd BODY (SUN) + SOLAR RADIATION PRESSURE
...
from poliastro.constants import rotational_period_sun, Wdivc_sun
from poliastro.core.perturbations import J2_perturbation, third_body, radiation_pressure
...
def j2_coesa_sun(t0, state, k, J2, R, C_D, A_over_m, k_third, r_third, C_R, W_over_c):
    return J2_perturbation(
        t0, state, k, J2, R
        ) + coesa76_model(
            state, R, C_D, A_over_m
            ) + third_body(
                t0, state, k, k_third, r_third
                ) + radiation_pressure(
                t0, state, k, R, C_R, A_over_m, W_over_c, r_third)

    
def f(t0, state, k):
    du_kep = func_twobody(t0, state, k)
    ax, ay, az = j2_coesa_sun(
        t0,
        state,
        k,
        R=R,
